...
function getCachedUnityStats(datePreset: string) {
  return unstable_cache(
    ...
    [`unity-insights-${datePreset}`],
    {
      revalidate: CACHE_DURATION,
      tags: ['unity-insights', `unity-insights-${datePreset}`],
    }
  )();
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await getCachedUnityStats(datePreset);
